...
This section describes how resources will be used to complete the defined project on time and within budget.
...
Table ~\ref{table:wbs} shows the \gls{wbs}. The major goals (1.1, 1.2, 1.3, etc.) correlate to the major milestones on the Gantt Chart.
...
\begin{longtable}{|c|m{4cm}|m{4cm}|>{\centering}m{1.6cm}|m{3.5cm}|}
	...
	\hline 1 & \multicolumn{4}{c|}{CNC Interface} \\ \hline
...
Table ~\ref{table:lrc} describes who will be working on which tasks. 
See Table ~\ref{table:wbs} to see the definition of each \gls{wbs} ID.
...
Figure ~\ref{fig:aon} shows the dependencies of all of the deliverables listed on the \gls{wbs}.
See Table ~\ref{table:wbs} to see the definition of each \gls{wbs} ID.
...
Figure ~\ref{fig:gantt} shows the current schedule of work based upon the \gls{lrc} in Table ~\ref{table:lrc} and the \gls{aon} in Figure ~\ref{fig:aon}.
...
\section{Budget}
Table ~\ref{table:budget} describes the current estimate for the budget for construction of one prototype.
Extra components will be ordered to mitigate risk, but add to the total budget for the project. 
...
Table ~\ref{table:engineeringhours} shows current estimates on how many hours each engineer will spend on the project.
The estimate was based on working 10 hours per week for developing, verifying, and managing the project with 15\% added as a safety measure. 
